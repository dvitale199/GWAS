...
# the following generates .assoc files for plink --meta-analysis method 
...
summary_data_list = [pd.read_csv('/data/vitaled2/summary_stats/rvtest_files/' + file, sep='\t', usecols=['markerID','beta','se','P']) for file in file_list]

summary_data_list_cleaned = []
for i in range(len(summary_data_list)):
    summary_data_list[i].sort_values('P').drop_duplicates(['markerID'], keep='first')
...
merged.to_csv("random_effect_test_5_cohorts.txt", index=False, header=False)
